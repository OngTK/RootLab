...
DROP TABLE IF EXISTS push;				# 푸시알림/팝업(*자체 테이블)
...
CREATE TABLE manager (
    ...
    mName VARCHAR(60) NOT NULL UNIQUE,                              -- 이름
...
CREATE TABLE push (
    ...
    pNo VARCHAR(15) NOT NULL,					        -- 플레이스No[FK]
    mgNo BINARY(16) NOT NULL,							-- 관리자No[FK]
    ...
    ppbIterated TIME NOT NULL							-- 푸시알림 시간(*09~20시 사이 분단위)
);
...
CREATE TABLE categoryCode (
    ...
    lclsSystm1Cd CHAR(3) NOT NULL, 								-- 대분류코드(#TourAPI 연동컬럼)
    lclsSystm1Nm VARCHAR(15) NOT NULL, 							-- 대분류명(#TourAPI 연동컬럼)
    lclsSystm2Cd CHAR(5) NOT NULL,								-- 중분류코드(#TourAPI 연동컬럼)
    lclsSystm2Nm VARCHAR(30) NOT NULL, 							-- 중분류명(#TourAPI 연동컬럼)
    lclsSystm3Cd CHAR(9) NOT NULL,								-- 소분류코드(#TourAPI 연동컬럼)
...
CREATE TABLE placeInfo (
	pNo INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,			-- place번호[PK]
    ...
    contentid INT UNIQUE DEFAULT NULL,						-- 콘텐츠ID(#TourAPI 연동컬럼)
...
CREATE TABLE markersGPS (
	mkNo INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,			-- 마커번호[PK]
    pNo INT UNSIGNED,										-- place번호[FK]
    mkURL VARCHAR(255),										-- 마커이미지경로
    mapx DECIMAL(13,10) DEFAULT NULL,						-- GPS X좌표(#TourAPI 연동컬럼)
    mapy DECIMAL(13,10) DEFAULT NULL,						-- GPS Y좌표(#TourAPI 연동컬럼)
    createdAt DATETIME DEFAULT CURRENT_TIMESTAMP,			-- 등록일
    updatedAt DATETIME DEFAULT NULL							-- 수정일
                 ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE placeImageDetail (
	pidNo INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,			-- 상세이미지번호[PK]
    pNo INT UNSIGNED,										-- place번호[FK]
    isEditable BOOLEAN DEFAULT TRUE,						-- 수정 가능 여부
    serialnum  VARCHAR(10),									-- 이미지일련번호(#TourAPI 연동컬럼)
    originimgurl  VARCHAR(255),								-- 원본이미지(#TourAPI 연동컬럼)
    smallimageurl  VARCHAR(255),							-- 썸네일이미지(#TourAPI 연동컬럼)
    imgname  VARCHAR(30),									-- 이미지명(#TourAPI 연동컬럼)
    createdAt DATETIME DEFAULT CURRENT_TIMESTAMP,			-- 등록일
    updatedAt DATETIME DEFAULT NULL							-- 수정일
                 ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE tourIntro (
	tNo INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,		-- 관광지소개번호[PK]
    pNo INT UNSIGNED,									-- place번호[FK]
...
CREATE TABLE placeInfoRepeat (
    ...
    infoname VARCHAR(10),									-- 제목(#TourAPI 연동컬럼)
